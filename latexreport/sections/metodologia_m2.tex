...
\texttt{FilteredClassifier}: il test fold non viene mai modificato perché rappresenta la
distribuzione reale del problema --- quella che il modello incontrerà in produzione.
...
\begin{itemize}
\item \textbf{SMOTE e oversampling:} SMOTE genera punti sintetici interpolando istanze
del training. Se applicato prima dello split, un punto sintetico potrebbe finire nel test
set, contaminandolo con dati derivati dall'insieme di addestramento.
\item \textbf{Feature selection:} se il calcolo degli score usa tutte le istanze, le
label del test fold influenzano la selezione, rendendola artificialmente buona.
\item \textbf{Undersampling:} la selezione delle istanze da rimuovere viene effettuata
osservando l'intero dataset, incluse le istanze del test set.
\end{itemize}

In tutti i casi le metriche stimate sembrano migliori di quelle reali. La soluzione
adottata è \texttt{FilteredClassifier} di Weka, che garantisce che ogni filtro
(balancing e feature selection) venga applicato internamente a ogni fold. L'ordine dei
filtri è rilevante: prima il balancing, poi la feature selection. La FS deve osservare
la distribuzione già bilanciata: calcolare InfoGain su un training sbilanciato (9\%
buggy) produrrebbe score dominati dalla classe maggioritaria.
...
selection) venga applicato \textbf{internamente per ogni fold}: il training fold è
l'unico input per il fit dei filtri, mentre il test fold viene solo trasformato (applicando
la selezione già calcolata sul training) senza mai influenzare il fit. Non vi è data
leakage: ogni predizione di test è prodotta da un modello che non ha mai visto quella
istanza.
...
\paragraph{Metrica di valutazione nei metodi wrapper: accuracy vs AUC}

\texttt{WrapperSubsetEval} usa per default l'\textbf{accuracy} come metrica interna per
confrontare i sottoinsiemi candidati durante la ricerca greedy. Su un dataset sbilanciato
come OpenJPA ($\approx$91\% non-buggy) questo scelta produce un comportamento degenere:
un modello che predice sempre la classe maggioritaria (ZeroR, zero feature) ottiene il
91\% di accuracy. \texttt{GreedyStepwise} con ricerca backward parte da tutte le 19
feature e rimuove iterativamente quella la cui eliminazione non abbassa la metrica
interna; poiché con accuracy la rimozione di ogni feature tende a non peggiorare il 91\%
di ZeroR, il processo converge al sottoinsieme vuoto. Il risultato osservato era il
messaggio Weka \textit{``Cannot build model (only class attribute present in data!)''}
per tutte le combinazioni BackwardSearch, con fallback automatico su ZeroR e metriche
degeneri nel CSV di output.

La soluzione consiste nel configurare \texttt{WrapperSubsetEval} con metrica
\textbf{AUC} (opzione \texttt{-E AUC}). Un classificatore ZeroR ottiene AUC$=0{,}5$
(potere discriminante nullo), mentre qualsiasi sottoinsieme che contenga almeno una
feature informativa ottiene AUC${>}0{,}5$. La ricerca greedy non può più convergere
all'insieme vuoto: eliminare l'ultima feature utile farebbe calare l'AUC in modo
rilevabile. Questa configurazione è disponibile come opzione nativa di
\texttt{WrapperSubsetEval} in Weka e non richiede modifiche al codice Weka: si tratta
di una scelta metodologica, non di un hack. La scelta è coerente con la metrica di
valutazione primaria adottata per i classificatori in questo studio: AUC è raccomandato
in letteratura rispetto all'accuracy per la feature selection su dataset sbilanciati,
per la stessa ragione per cui viene preferito all'accuracy nella valutazione finale.
