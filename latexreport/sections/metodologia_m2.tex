...
\medskip
\noindent\textbf{Nota sui tempi:} i metodi wrapper sono \textbf{lenti} (1--3h per
combinazione) e abilitabili o disabilitabili in \texttt{config.yaml}. Con 19 feature e 10$\times$10 fold CV esterna, ForwardSearch
...
L'ordine dei filtri è: prima il balancing, poi la feature selection. Entrambi gli
ordini sono usati in letteratura; quello adottato garantisce che la FS calcoli gli
score su una distribuzione bilanciata, evitando che la sproporzione 9\%/91\% comprima
i valori di correlazione della classe minoritaria. Con la soglia \texttt{threshold=0.0}
usata in questo progetto la differenza pratica tra i due ordini è comunque trascurabile:
vengono eliminate solo le feature con score esattamente zero, indipendentemente dall'ordine.
...
configurazione attuale esegue 60 combinazioni (3 clf $\times$ 5 FS $\times$ 4 bal, tutte
le strategie abilitate). Disabilitando SMOTE e i metodi wrapper si scende a 27
combinazioni (3 clf $\times$ 3 FS $\times$ 3 bal).
...
\midrule
Minimo (SMOTE e wrapper disabilitati, 3 clf $\times$ 3 FS $\times$ 3 bal) & 27 & $\approx$35 min \\
+ SMOTE (3 clf $\times$ 3 FS $\times$ 4 bal) & 36 & $\approx$1 ora \\
+ ForwardSearch o BackwardSearch & $+$9 ciascuno & $+$9--27 ore \\
Tutto abilitato --- configurazione attuale (3 clf $\times$ 5 FS $\times$ 4 bal) & 60 & 1--3 giorni \\
\bottomrule
\end{tabular}
