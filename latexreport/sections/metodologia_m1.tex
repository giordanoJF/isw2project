...
\begin{itemize}
\item \texttt{RepoCloneService}: esegue un \textbf{full clone} tramite \textbf{JGit}
(libreria Java pura, senza dipendenze dal client \texttt{git} installato sul sistema).
...
\medskip
\noindent\textbf{Correzione: shallow clone causa un'etichettatura errata della bugginess.}
La versione iniziale di \texttt{RepoCloneService} eseguiva uno \emph{shallow clone}
(\texttt{depth=1}), che scarica solo l'ultimo snapshot di ogni branch senza storia dei
commit. Questo dimezzava il tempo di download ma causava un bug silenzioso nella pipeline:
\texttt{CommitIndexService} e \texttt{GitLogStatsService} scandiscono l'intera storia dei
commit per costruire gli indici bug\-fix e file\-change; con uno shallow clone quei
servizi vedevano meno del 10\% dei commit reali. Il risultato osservato era un tasso di
bugginess del $\approx$60\% invece dell'atteso $\approx$9\%: quasi tutti i file
risultavano ``buggy'' semplicemente perché la maggior parte dei commit che li citavano
era assente dall'indice. La correzione consiste nel passare a un full clone: il download
è più pesante ($\approx$100\,MB anziché pochi MB) ma i dati estratti rispecchiano
correttamente la storia reale del progetto.
