...
/** Average change set size per revision up to the release. */
public class AvgChangeSetMetric implements Metric {

    private final GitLogStatsService gitLogStatsService;

    public AvgChangeSetMetric(GitLogStatsService gitLogStatsService) {
        this.gitLogStatsService = gitLogStatsService;
    }

    @Override
    public String columnName() { return "Avg_Change_Set"; }
    ...
    public String compute(ClassMetricInput input) throws GitCommandException {
        ...
        double avg = stats.stream().mapToInt(GitLogStatsService.CommitStat::getChangeSetSize).average().orElse(0);
