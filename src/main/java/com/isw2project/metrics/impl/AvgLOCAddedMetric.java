...
/** Average lines added per revision up to the release. */
public class AvgLOCAddedMetric implements Metric {

    private final GitLogStatsService gitLogStatsService;

    public AvgLOCAddedMetric(GitLogStatsService gitLogStatsService) {
        this.gitLogStatsService = gitLogStatsService;
    }

    @Override
    public String columnName() { return "Avg_LOC_Added"; }
    ...
    public String compute(ClassMetricInput input) throws GitCommandException {
        ...
        double avg = stats.stream().mapToInt(GitLogStatsService.CommitStat::getLinesAdded).average().orElse(0);
