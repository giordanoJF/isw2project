...
/** Sum of added and deleted lines (churn) across all revisions up to the release. */
public class ChurnMetric implements Metric {

    private final GitLogStatsService gitLogStatsService;

    public ChurnMetric(GitLogStatsService gitLogStatsService) {
        this.gitLogStatsService = gitLogStatsService;
    }

    @Override
    public String columnName() { return "Churn"; }
    ...
    public String compute(ClassMetricInput input) throws GitCommandException {
        ...
        int total = stats.stream().mapToInt(GitLogStatsService.CommitStat::getChurn).sum();
